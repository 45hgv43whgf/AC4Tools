...
$iniPath = Join-Path $GameDir "scripts\AC4Tools.ini"
$oldConfig = $null
...
$asiPath = Join-Path $GameDir "scripts\AC4Tools.asi"
try {
    Copy-Item -LiteralPath (Join-Path $releaseDir "scripts\AC4Tools.asi") -Destination $asiPath -Force
} catch {
    throw "Could not update scripts\AC4Tools.asi because it is locked. Close Assassin's Creed IV Black Flag, then run install_to_game.ps1 again. The INI was not changed."
}
...
$defaultIniPath = Join-Path $releaseDir "scripts\AC4Tools.ini"
$tempIniPath = Join-Path (Join-Path $GameDir "scripts") ("AC4Tools.ini.tmp-{0}" -f [Guid]::NewGuid().ToString("N"))
